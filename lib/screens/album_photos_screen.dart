...
class _AlbumPhotosScreenState extends State<AlbumPhotosScreen> {
  ...
  void _startSelection(Photo photo) {
    ...
    setState(() {
      ...
      _isSwipeSelecting = false;
      _swipeStartIndex = index < 0 ? null : index;
      _swipePointerStart = _pointers.isEmpty ? null : _pointers.values.first;
      _swipeDidMove = false;
      _swipeGestureMode = null;
      _swipeLastPosition = _swipePointerStart;
      _swipeBaselineSelection = Set<int>.from(_selectedPhotoIds);
    ...
    if (from < to) {
      ...
      for (var row = startRow + 1; row < endRow; row++) {
        final s = row * _crossAxisCount,
            e = math.min(_photos.length - 1, s + _crossAxisCount - 1);
        for (var i = s; i <= e; i++) result.add(i);
      }
      final lastStart = endRow * _crossAxisCount;
      for (var i = lastStart; i <= to; i++) result.add(i);
    } else {
      final firstStart = startRow * _crossAxisCount;
      for (var i = firstStart; i <= from; i++) result.add(i);
      for (var row = startRow - 1; row > endRow; row--) {
        final s = row * _crossAxisCount,
            e = math.min(_photos.length - 1, s + _crossAxisCount - 1);
        for (var i = s; i <= e; i++) result.add(i);
      ...
      for (var i = to; i <= lastEnd; i++) result.add(i);
    }
    return result;
  }
  ...
  void _pointerDown(PointerDownEvent event) {
    ...
    if (_isSelectionMode && !_isPinching) {
      ...
      if (index != null) {
        _swipeStartIndex = index;
        _swipePointerStart = event.position;
        _swipeDidMove = false;
        _swipeGestureMode = null;
    ...
    _selectionAutoScrollTimer = Timer.periodic(
      ...
      (_) {
        ...
        if ((target - _scrollController.offset).abs() > .1) {
          ...
          final index = _indexAt(p);
          if (index != null) _applySwipeSelection(index);
        } else {
          _stopSelectionAutoScroll();
        }
      },
  ...
  _GridAnchor _captureGridAnchor(int columns) {
    if (!_scrollController.hasClients || _photos.isEmpty)
      return const _GridAnchor(0, 0);
  ...
  Future<void> _showAlbumPicker() async {
    ...
    try {
      ...
      final selected = await _showAlbumSelectionDialog(
        title: 'Add to album',
        albums: available,
      );
      if (selected != null && mounted)
        await _addSelectedPhotosToAlbum(selected);
    } catch (e) {
      if (!mounted) return;
      setState(() => _isAddingToAlbum = false);
  ...
  Future<void> _showMoveAlbumPicker() async {
    ...
    try {
      ...
      if (selected != null && mounted)
        await _moveSelectedPhotosToAlbum(selected);
    } catch (e) {
      if (mounted)
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('Failed to load albums: $e')));
  ...
  Future<void> _moveSelectedPhotosToAlbum(Album destination) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          failed == 0
              ? '$moved photo${moved == 1 ? '' : 's'} moved to "${destination.name}".'
  ...
  Future<void> _removeSelectedPhotos() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          count == 1
              ? 'Remove this photo from "${widget.albumName}"?'
              : 'Remove $count photos from "${widget.albumName}"?',
        ),
        actions: [
          TextButton(
    ...
    for (final id in ids) {
      try {
        await _apiService.removePhotoFromAlbum(widget.albumId!, id);
        removed++;
      } catch (_) {
        failed++;
      }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          failed == 0
              ? '$removed photo${removed == 1 ? '' : 's'} removed from album.'
  ...
  Future<void> _moveSelectedPhotosToTrash() async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          count == 1
              ...
              : '$count photos will be moved to Trash. You can restore them for 30 days.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
    ...
    for (final id in ids) {
      try {
        await _apiService.movePhotoToTrash(id);
        moved++;
      } catch (_) {
        failed++;
      }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          failed == 0
              ? '$moved photo${moved == 1 ? '' : 's'} moved to Trash.'
              : '$moved moved to Trash, $failed failed.',
        ),
      ),
    );
  }

  Future<Album?> _showAlbumSelectionDialog({
    ...
    return AppBar(
      ...
      actions: [
        ...
        IconButton(
          ...
          onPressed: count == 0 || _isAddingToAlbum ? null : _showAlbumPicker,
          icon: _isAddingToAlbum
              ? const SizedBox(
                  width: 20,
                  height: 20,
        ...
        if (!widget.isRecent) ...[
          IconButton(
            ...
            onPressed: count == 0 || _isAddingToAlbum
                ? null
          ...
          IconButton(
            ...
            onPressed: count == 0 || _isRemoving ? null : _removeSelectedPhotos,
      ...
      return RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            Center(
              child: Text(
                widget.isRecent ? 'No recent photos' : 'This album is empty',
