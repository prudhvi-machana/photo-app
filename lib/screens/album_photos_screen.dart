...
class _AlbumPhotosScreenState extends State<AlbumPhotosScreen>{
  ...
  bool _isLoading=true,_isSelectionMode=false,_isRemoving=false,_isAddingToAlbum=false,_isPinching=false,_isSwipeSelecting=false;
  String? _errorMessage;
  int _crossAxisCount=3,_pinchStartColumns=3;
  ...
  Future<void> _loadPhotos()async{
    try{
      final photos=widget.isRecent?await _apiService.getRecentPhotos():await _apiService.getAlbumPhotos(widget.albumId!);
      if(!mounted)return;
      setState((){_photos=photos;_isLoading=false;_errorMessage=null;});
    }catch(_){
      if(!mounted)return;
      setState((){_isLoading=false;_errorMessage=widget.isRecent?'Failed to load recent photos.':'Failed to load album photos.';});
    }
  }
  ...
  void _openPhoto(Photo photo,int index){
    if(_isSelectionMode){_toggleSelection(photo);return;}
    Navigator.of(context).push(MaterialPageRoute(builder:(_)=>PhotoViewerScreen(photos:_photos,initialIndex:index,token:widget.token))).then((result){if(result==true&&mounted)_loadPhotos();});
  }
  ...
  void _toggleSelection(Photo photo){
    setState((){
      if(_selectedPhotoIds.contains(photo.id))_selectedPhotoIds.remove(photo.id);else _selectedPhotoIds.add(photo.id);
  ...
  void _selectAll(){
    setState((){
      ...
      if(allSelected)_selectedPhotoIds.clear();else _selectedPhotoIds..clear()..addAll(_photos.map((p)=>p.id));
    });
  }
  ...
  int? _indexAt(Offset globalPosition){
    for(var i=0;i<_photos.length;i++){
      final object=_tileKey(i).currentContext?.findRenderObject();
      if(object is! RenderBox)continue;
      final topLeft=object.localToGlobal(Offset.zero);
      if((topLeft&object.size).contains(globalPosition))return i;
    }
    return null;
  }
  ...
  Future<void> _showAlbumPicker()async{
    ...
    try{
      ...
      final available=widget.isRecent?albums:albums.where((a)=>a.id!=widget.albumId).toList();
      if(available.isEmpty){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('No other albums available.')));return;}
    ...
    }catch(e){if(!mounted)return;setState(()=>_isAddingToAlbum=false);ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Failed to load albums: $e')));}
  }
  Future<void> _addSelectedPhotosToAlbum(Album destination)async{
    final ids=List<int>.from(_selectedPhotoIds);setState(()=>_isAddingToAlbum=true);var added=0,already=0,failed=0;
    for(final id in ids){try{await _apiService.addPhotoToAlbum(destination.id,id);added++;}catch(e){if(e.toString().contains('409'))already++;else failed++;}}
    if(!mounted)return;setState((){_isAddingToAlbum=false;_selectedPhotoIds.clear();_isSelectionMode=false;});
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(failed==0&&already==0?'$added photo${added==1?'':'s'} added to "${destination.name}".':'$added added, $already already there, $failed failed.')));
  }
  Future<void> _showMoveAlbumPicker()async{
    ...
    try{
      final albums=await _apiService.getAlbums();if(!mounted)return;
      final available=albums.where((a)=>a.id!=widget.albumId).toList();
      if(available.isEmpty){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('No other albums available.')));return;}
      final selected=await _showAlbumSelectionDialog(title:'Move to album',albums:available);if(selected!=null&&mounted)await _moveSelectedPhotosToAlbum(selected);
    }catch(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Failed to load albums: $e')));}
  }
  Future<void> _moveSelectedPhotosToAlbum(Album destination)async{
    if(widget.albumId==null)return;final ids=List<int>.from(_selectedPhotoIds);setState(()=>_isAddingToAlbum=true);var moved=0,failed=0;
    for(final id in ids){try{await _apiService.movePhotoToAlbum(widget.albumId!,destination.id,id);moved++;}catch(_){failed++;}}
    if(!mounted)return;setState((){_isAddingToAlbum=false;if(failed==0)_photos.removeWhere((p)=>ids.contains(p.id));_selectedPhotoIds.clear();_isSelectionMode=false;});
  ...
  Future<void> _removeSelectedPhotos()async{
    ...
    final count=_selectedPhotoIds.length;final confirmed=await showDialog<bool>(context:context,builder:(ctx)=>AlertDialog(title:const Text('Remove from album?'),content:Text(count==1?'Remove this photo from "${widget.albumName}"?':'Remove $count photos from "${widget.albumName}"?'),actions:[TextButton(onPressed:()=>Navigator.pop(ctx,false),child:const Text('Cancel')),FilledButton(onPressed:()=>Navigator.pop(ctx,true),child:const Text('Remove'))]));
    if(confirmed!=true)return;setState(()=>_isRemoving=true);final ids=List<int>.from(_selectedPhotoIds);var removed=0,failed=0;
    for(final id in ids){try{await _apiService.removePhotoFromAlbum(widget.albumId!,id);removed++;}catch(_){failed++;}}
    if(!mounted)return;setState((){_photos.removeWhere((p)=>ids.contains(p.id));_selectedPhotoIds.clear();_isSelectionMode=false;_isRemoving=false;});
  ...
  Future<void> _moveSelectedPhotosToTrash()async{
    if(_selectedPhotoIds.isEmpty||_isRemoving)return;final count=_selectedPhotoIds.length;final confirmed=await showDialog<bool>(context:context,builder:(ctx)=>AlertDialog(title:const Text('Move to Trash?'),content:Text(count==1?'This photo will be moved to Trash. You can restore it for 30 days.':'$count photos will be moved to Trash. You can restore them for 30 days.'),actions:[TextButton(onPressed:()=>Navigator.pop(ctx,false),child:const Text('Cancel')),FilledButton(onPressed:()=>Navigator.pop(ctx,true),child:const Text('Move to Trash'))]));
    if(confirmed!=true)return;setState(()=>_isRemoving=true);final ids=List<int>.from(_selectedPhotoIds);var moved=0,failed=0;
    for(final id in ids){try{await _apiService.movePhotoToTrash(id);moved++;}catch(_){failed++;}}
    if(!mounted)return;setState((){_photos.removeWhere((p)=>ids.contains(p.id));_selectedPhotoIds.clear();_isSelectionMode=false;_isRemoving=false;});
  ...
  Future<Album?> _showAlbumSelectionDialog({required String title,required List<Album> albums})=>showDialog<Album>(context:context,builder:(ctx)=>AlertDialog(title:Text(title),content:SizedBox(width:double.maxFinite,child:ListView.separated(shrinkWrap:true,itemCount:albums.length,separatorBuilder:(_,_)=>const Divider(height:1),itemBuilder:(_,index){final album=albums[index];return ListTile(leading:const Icon(Icons.photo_album_outlined),title:Text(album.name,maxLines:1,overflow:TextOverflow.ellipsis),subtitle:Text(album.photoCount==1?'1 photo':'${album.photoCount} photos'),trailing:const Icon(Icons.chevron_right),onTap:()=>Navigator.pop(ctx,album));})),actions:[TextButton(onPressed:()=>Navigator.pop(ctx),child:const Text('Cancel'))]));

  PreferredSizeWidget _buildAppBar(){
    if(!_isSelectionMode)return AppBar(title:Text(widget.albumName),actions:[if(_photos.isNotEmpty)IconButton(tooltip:'Select photos',onPressed:()=>setState(()=>_isSelectionMode=true),icon:const Icon(Icons.checklist))]);
    final count=_selectedPhotoIds.length;final allSelected=_photos.isNotEmpty&&count==_photos.length;
    return AppBar(leading:IconButton(tooltip:'Cancel selection',onPressed:_clearSelection,icon:const Icon(Icons.close)),title:Text('$count selected'),actions:[TextButton(onPressed:_selectAll,child:Text(allSelected?'Deselect all':'Select all')),IconButton(tooltip:'Add to album',onPressed:count==0||_isAddingToAlbum?null:_showAlbumPicker,icon:_isAddingToAlbum?const SizedBox(width:20,height:20,child:CircularProgressIndicator(strokeWidth:2)):const Icon(Icons.library_add_outlined)),if(!widget.isRecent)...[IconButton(tooltip:'Move to album',onPressed:count==0||_isAddingToAlbum?null:_showMoveAlbumPicker,icon:const Icon(Icons.drive_file_move_outlined)),IconButton(tooltip:'Remove from album',onPressed:count==0||_isRemoving?null:_removeSelectedPhotos,icon:const Icon(Icons.remove_circle_outline))],IconButton(tooltip:'Move to Trash',onPressed:count==0||_isRemoving?null:_moveSelectedPhotosToTrash,icon:const Icon(Icons.delete_outline))]);
  }
...
class _GridAnchor{final int index;final double offset;const _GridAnchor(this.index,this.offset);}
