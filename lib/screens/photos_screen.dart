...
class _PhotosScreenState extends State<PhotosScreen> {
    ...
    try {
      ...
      if (permission.hasAccess) {
        ...
        while (true) {
          ...
          assets.addAll(page.where((asset) => !asset.isTrashed));
          if (page.length < pageSize) break;
          start += page.length;
        }

        // Fetch titles concurrently in batches instead of waiting for every
        // MediaStore query sequentially. This is much faster for large libraries.
        const batchSize = 100;
        for (var i = 0; i < assets.length; i += batchSize) {
          final end = math.min(i + batchSize, assets.length);
          final batch = assets.sublist(i, end);
          final names = await Future.wait(batch.map((asset) => asset.titleAsync()));
          for (var j = 0; j < batch.length; j++) {
            local.add(LocalMedia(asset: batch[j], filename: names[j]));
          }
        }
      }
      ...
      final cloudNames = <String>{
        for (final photo in cloud) photo.originalFilename.trim().toLowerCase(),
      };
      local = [
  ...
  Map<DateTime, List<_MediaItem>> _buildGroups(List<Photo> cloud, List<LocalMedia> local) {
    ...
    final localNames = {for (final item in local) item.filename.trim().toLowerCase()};
    for (final photo in cloud) {
      items.add(_MediaItem.cloud(
        photo,
    ...
    for (final item in local) {
      if (!cloudNames.contains(item.filename.trim().toLowerCase())) {
        items.add(_MediaItem.local(item));
      }
    }
  ...
  void _selectAll() {
    final all = [for (final list in _mediaGroups.values) ...list];
  ...
  Future<void> _moveSelectedToTrash() async {
    ...
    if (selected.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Only cloud photos can be moved to Trash.')),
      );
      return;
    }
    final cloudCount = selected.length;
    final totalCount = _selectedKeys.length;
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Move to Trash?'),
        content: Text(
          'Move $cloudCount cloud ${cloudCount == 1 ? 'photo' : 'photos'} '
          'out of $totalCount selected ${totalCount == 1 ? 'file' : 'files'} '
          'to Trash? You can restore them for 30 days.',
        ),
        actions: [
    ...
    for (final photo in selected) {
      try {
        await _apiService.movePhotoToTrash(photo.id);
        success++;
      } catch (_) {
        failed++;
      }
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(failed == 0 ? '$success photo${success == 1 ? '' : 's'} moved to Trash.' : '$success moved, $failed failed.')),
    );
  }
  ...
  Future<void> _addSelectedToAlbum() async {
    final selected = _selectedCloudPhotos;
    if (selected.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Only cloud photos can be added to albums.')));
      return;
    }
    setState(() => _isActionRunning = true);
    List<Album> albums;
    ...
    for (final photo in selected) {
      try {
        await _apiService.addPhotoToAlbum(album.id, photo.id);
        added++;
      } catch (e) {
  ...
  void _openCloudPhoto(Photo photo) => Navigator.of(context).push(MaterialPageRoute(
    builder: (_) => PhotoViewerScreen(photos: _cloudPhotos, initialIndex: _cloudPhotos.indexOf(photo), token: widget.token),
  ));
  void _openLocalPhoto(LocalMedia media) => Navigator.of(context).push(MaterialPageRoute(
    builder: (_) => LocalMediaViewerScreen(asset: media.asset),
  ));
  Future<void> _openTrash() async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => TrashScreen(token: widget.token)));
    if (mounted) await _refresh();
  }
  Future<void> _openUploadPhotos() async {
    final uploaded = await Navigator.of(context).push<bool>(MaterialPageRoute(builder: (_) => UploadPhotosScreen(token: widget.token)));
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      leading: _isSelectionMode ? IconButton(tooltip: 'Close selection', icon: const Icon(Icons.close), onPressed: _clearSelection) : null,
    ...
    floatingActionButton: _isSelectionMode ? null : FloatingActionButton(tooltip: 'Add photos', onPressed: _openUploadPhotos, child: const Icon(Icons.add)),
    body: _buildBody(),
  );
  ...
  Widget _buildBody() {
    ...
    final dates = _mediaGroups.keys.toList();
    return Listener(
            ...
            slivers: [
              ...
              for (final date in dates) ...[
                ...
                SliverPadding(
                  ...
                  sliver: SliverGrid(
                    delegate: SliverChildBuilderDelegate((context, index) => _buildMediaTile(_mediaGroups[date]![index]), childCount: _mediaGroups[date]!.length),
                    gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: _crossAxisCount, crossAxisSpacing: 2, mainAxisSpacing: 2, childAspectRatio: 1),
                  ),
                ),
              ],
  ...
  Widget _buildStatusBadge({required bool local, required bool cloud}) {
    ...
    return Positioned(top: 5, right: 5, child: Tooltip(message: label, child: Container(padding: const EdgeInsets.all(5), decoration: BoxDecoration(color: Colors.black.withValues(alpha: .62), shape: BoxShape.circle), child: Icon(icon, color: Colors.white, size: 16))));
  }
}
