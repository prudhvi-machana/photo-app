...
class _PhotosScreenState extends State<PhotosScreen> {
    ...
    try {
      ...
      if(permission.hasAccess){
        ...
        while(true){
          ...
          allAssets.addAll(page.where((asset)=>!asset.isTrashed));
          if(page.length<pageSize)break;
          start+=page.length;
        }
        local=[for(final asset in allAssets)LocalMedia(asset:asset,filename:await asset.titleAsync)];
      }
      final cloud=await cloudFuture;
      final cloudNames={for(final photo in cloud)photo.originalFilename.trim().toLowerCase()};
      local=[for(final item in local)LocalMedia(asset:item.asset,filename:item.filename,alsoInCloud:cloudNames.contains(item.filename.trim().toLowerCase()))];
      if(!mounted)return;
      setState((){_cloudPhotos=cloud;_localMedia=local;_hasLocalPermission=permission.hasAccess;_isLoading=false;_errorMessage=null;});
  ...
  List<Photo> get _selectedCloudPhotos=>[for(final photo in _cloudPhotos)if(_selectedKeys.contains('cloud:${photo.id}'))photo];
  ...
  void _handleTap(_MediaItem item){if(_isSelectionMode){_toggleSelection(item);return;}if(item.isCloud)_openCloudPhoto(item.cloud!);else _openLocalPhoto(item.local!);}
  void _clearSelection(){setState((){_selectedKeys.clear();_isSelectionMode=false;});}
  void _selectAll(){final groups=_groupMediaByDate();final all=[for(final list in groups.values)...list];final allSelected=all.isNotEmpty&&_selectedKeys.length==all.length;setState((){_isSelectionMode=true;_selectedKeys.clear();if(!allSelected)_selectedKeys.addAll(all.map(_keyFor));});}

  Future<void> _moveSelectedToTrash() async {
    final selected=_selectedCloudPhotos;
    if(selected.isEmpty){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Only cloud photos can be moved to Trash.')));return;}
    final cloudCount=selected.length,totalCount=_selectedKeys.length;
    final confirmed=await showDialog<bool>(context:context,builder:(ctx)=>AlertDialog(
      title:const Text('Move to Trash?'),
      content:Text('Move $cloudCount cloud ${cloudCount==1?'photo':'photos'} out of $totalCount selected ${totalCount==1?'file':'files'} to Trash? You can restore them for 30 days.'),
      actions:[TextButton(onPressed:()=>Navigator.pop(ctx,false),child:const Text('Cancel')),FilledButton(onPressed:()=>Navigator.pop(ctx,true),child:const Text('Move to Trash'))],
    ));
    if(confirmed!=true)return;
    setState(()=>_isActionRunning=true);int success=0,failed=0;
    for(final photo in selected){try{await _apiService.movePhotoToTrash(photo.id);success++;}catch(_){failed++;}}
    if(!mounted)return;setState(()=>_isActionRunning=false);_clearSelection();await _refresh();if(!mounted)return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(failed==0?'$success photo${success==1?'':'s'} moved to Trash.':'$success moved, $failed failed.')));
  }

  Future<void> _addSelectedToAlbum() async {
    final selected=_selectedCloudPhotos;
    if(selected.isEmpty){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Only cloud photos can be added to albums.')));return;}
    setState(()=>_isActionRunning=true);List<Album> albums;
    try{albums=await _apiService.getAlbums();}catch(e){if(!mounted)return;setState(()=>_isActionRunning=false);ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Failed to load albums: $e')));return;}
    if(!mounted)return;setState(()=>_isActionRunning=false);if(albums.isEmpty){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('No albums available.')));return;}
    final album=await showDialog<Album>(context:context,builder:(ctx)=>AlertDialog(title:const Text('Add to album'),content:SizedBox(width:double.maxFinite,height:math.min(360,albums.length*56.0),child:ListView.builder(itemCount:albums.length,itemBuilder:(_,index){final album=albums[index];return ListTile(leading:const Icon(Icons.photo_album_outlined),title:Text(album.name),onTap:()=>Navigator.pop(ctx,album));})),actions:[TextButton(onPressed:()=>Navigator.pop(ctx),child:const Text('Cancel'))],));
    if(album==null||!mounted)return;setState(()=>_isActionRunning=true);int added=0,already=0,failed=0;for(final photo in selected){try{await _apiService.addPhotoToAlbum(album.id,photo.id);added++;}catch(e){if(e.toString().contains('409'))already++;else failed++;}}if(!mounted)return;setState(()=>_isActionRunning=false);_clearSelection();final message=failed==0&&already==0?'$added photo${added==1?'':'s'} added to "${album.name}".':'$added added, $already already there, $failed failed.';ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(message)));
  }
