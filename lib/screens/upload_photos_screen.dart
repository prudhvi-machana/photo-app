...
class _UploadPhotosScreenState extends State<UploadPhotosScreen> {
  ...
  Future<void> _selectMedia() async {
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to select media.')));
    }
  }

  Future<void> _uploadSelectedMedia() async {
    if (_selectedMedia.isEmpty || _isQueueing) return;

    setState(() => _isQueueing = true);
    try {
      final items = _selectedMedia
          .map(
            (media) => MediaTransferItem(
              path: media.path,
              filename: media.name,
              isVideo: _isVideo(media),
            ),
          )
          .toList();
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Could not start upload: $e')));
    }
  }
        ...
        child: Column(
          children: [
            ...
            if (!hasSelection)
              ...
              Row(
                children: [
                  Expanded(child: OutlinedButton(onPressed: _isQueueing ? null : _selectMedia, child: const Text('Change'))),
                  const SizedBox(width: 12),
                  Expanded(
                    child: FilledButton(
                      onPressed: _isQueueing ? null : _uploadSelectedMedia,
                      child: _isQueueing ? const Text('Starting…') : Text('Upload ${_selectedMedia.length}'),
                    ),
                  ),
                ],
              ),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            widget.albumId != null ? 'Select photos and videos to add' : 'Select photos and videos to upload',
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ClipRRect(
          ...
          child: Image.file(File(media.path), fit: BoxFit.cover, errorBuilder: (context, error, stackTrace) => const Center(child: Icon(Icons.image))),
        );
      },
    );
  }
